  ...
  try {
    ...
    if (!user) {
      ...
      if(req.body.email && req.body.email !== '@'){
        test = await User.findOne({ where: { email: req.body.email } })
      }

      if(req.body.country_code && req.body.phone_number){
        test1 = await User.findOne({ where: { country_code: req.body.country_code, phone_number: req.body.phone_number } });
      }
      if(test){
        ...
        user.email = req.body.email && req.body.email !== '@' ? email : user.email;
...
exports.postOrder = async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      const store = await Store.findOne({ where: { catererId: catererId } });
